...
Der Tab `Störaufschaltung` simuliert eine additive Störung wahlweise als `d_u` am Streckeneingang oder als `d_y` am Streckenausgang. Der Störort kann direkt im Tab oder unter `Einstellungen > Störaufschaltung` gewählt werden; die nicht ausgewählte Störung wird im Regelkreisdiagramm ausgegraut. Amplitude, Startzeit, Ausregel-Toleranz und Komponentenanzeige liegen unter `Einstellungen > Störaufschaltung`. Angezeigt werden Ausgang `y(t)`, Reglerausgang `u_R(t)`, Störsignal und der resultierende Streckeneingang `u(t)` inklusive einer einfachen Ausregelzeit-Schätzung nach dem Störsprung.

Die Wurzelortskurve zeigt die geschlossenen Pole von `1 + K_WOK L_0(s) = 0` für einen einstellbaren Verstärkungsbereich, wobei `L_0(s)` der offene Kreis mit `K_WOK = 1` ist. Richtungspfeile, offene Pole und Nullstellen, markierte geschlossene Pole für den ausgewählten Gain, Stabilitätsauswertung, Hover-Daten, Dämpfungslinien, optionale Konstruktionshilfen mit Asymptoten samt Winkelangaben, Wurzelschwerpunkt, Ein- und Austrittswinkeln, Referenzlinien und Rechtwinkelmarken an Verzweigungen/Vereinigungen sowie eine optionale Padé-Approximation der Totzeit sind integriert. Fehlt `K_WOK`, fragt der Explorer, ob der Faktor ergänzt werden soll. Der markierte Entwurfswert kann im WOK-Tab eingegeben und mit Enter übernommen oder durch Klick auf die WOK ausgewählt werden.

Aktuelle Modelle können über `Datei > Als MATLAB-Skript exportieren...` als lauffähiges `.m`-Skript mit Nyquist-Ortskurve, Bode-Diagramm, Wurzelortskurve, Sprungantwort und Störaufschaltung exportiert werden. Der Export nutzt MATLAB-native Totzeitmodelle für Sprungantwort und Störaufschaltung; die Wurzelortskurve verwendet Padé nur optional, wenn die Totzeit dort aktiviert ist.
